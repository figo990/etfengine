...
with st.expander("💡 使用说明", expanded=False):
    st.markdown("""
    本页展示 **美股科技龙头** 最近财季的 **SEC EDGAR 结构化季报指标**
    （营收、净利润、稀释 EPS 等）及 **同比变化**，
    并给出面向跨境 ETF 的中文要点/解读（可选 LLM）。

    **数据如何更新**
    - 推荐：每周运行 `python scripts/update_overseas_earnings.py`，
      或依赖调度器中的「外盘科技龙头季报」周任务。
    - 首次使用若表为空，可点击下方 **「从 SEC 拉取并入库」**。

    **合规与网络**
    - SEC 要求请求携带合规 **User-Agent**（含联系信息）。
      请在 `config/overseas_earnings.yaml` 中修改 `sec_user_agent`，
      或设置环境变量 `SEC_EDGAR_USER_AGENT`。
    - **LLM 解读**（可选）：与智能资讯共用 `DEEPSEEK_API_KEY`；
      若未配置，将仅显示规则生成的中文事实摘要。
...
for col in ("revenue_usd", "net_income_usd"):
    if col in _disp.columns:
        _disp[col] = _disp[col].apply(lambda x: f"{x / 1e8:.2f}亿USD" if pd.notna(x) else "—")
if "eps_diluted" in _disp.columns:
    _disp["eps_diluted"] = _disp["eps_diluted"].apply(lambda x: f"{x:.2f}" if pd.notna(x) else "—")
for col in ("revenue_yoy_pct", "net_income_yoy_pct"):
    if col in _disp.columns:
        _disp[col] = _disp[col].apply(lambda x: f"{x:+.1f}%" if pd.notna(x) else "—")
...
st.dataframe(_disp[show_cols], width="stretch", hide_index=True)
