...
tab_dca, tab_grid, tab_rotation, tab_backtest, tab_batch, tab_results, tab_compare = st.tabs(
    [
        "定投信号",
        "网格设计",
        "轮动观察",
...
with tab_dca:
    ...
    if df.empty:
        ...
        dca_cols[1].metric("数据日期", str(latest["trade_date"]))
        ...
        if ma is not None:
            ...
            fig.add_trace(go.Scatter(x=recent["trade_date"], y=ma_series, name=f"MA{ma_period}"))
...
with tab_grid:
    ...
    if df.empty:
        ...
        if upper <= lower:
            ...
            st.plotly_chart(fig, width="stretch")
            st.dataframe(
                pd.DataFrame({"网格序号": range(len(lines)), "价格": [round(x, 4) for x in lines]}),
                width="stretch",
                hide_index=True,
            )
