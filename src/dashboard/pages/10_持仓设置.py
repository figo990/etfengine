...
with col2:
    total_capital = st.number_input("总资金(元)", value=portfolio.get("total_capital", 100000),
...
if not holdings:
    ...
    for i, h in enumerate(holdings):
        with st.container():
            ...
            with cols[1]:
                new_name = st.text_input("名称", value=h.get("name", ""), key=f"name_{i}",
...
with rc1:
    trigger = st.selectbox("再平衡触发方式",
                            ["drift", "periodic", "both"],
                            index=["drift", "periodic", "both"].index(rebalance.get("trigger", "drift")))
    drift_threshold = st.slider("偏离阈值(%)", 1, 20, int(rebalance.get("drift_threshold", 0.05) * 100))
    freq = st.selectbox("定期检查频率",
                          ["monthly", "weekly", "quarterly"],
                          index=["monthly", "weekly", "quarterly"].index(rebalance.get("periodic_frequency", "monthly")))

with rc2:
    max_dd = st.slider("最大回撤预警(%)", 5, 50, int(risk_limits.get("max_drawdown_alert", 0.15) * 100))
    max_pos = st.slider("单ETF最大仓位(%)", 10, 100, int(risk_limits.get("max_single_position", 0.40) * 100))
    min_pos = st.slider("单ETF最小仓位(%)", 1, 30, int(risk_limits.get("min_single_position", 0.05) * 100))
